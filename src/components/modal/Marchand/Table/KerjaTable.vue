<template>
    <Dialog :visible="dispModal" :modal="true" :closable="false" style="width: 90vh">
        <template #header>
            <h3 v-if="form.id" class="m-0"><span class="text-dgas-red">Edit Row - {{ form.attributes.name }}</span></h3>
        ...
        <form ref="form">
            <div class="formgrid grid">
                <div class="field col-12 md:col-12">
                    <label><span>Nama Pekerja</span></label>
                    <InputText id="name" v-model="formCopy.attributes.name"
                        class="text-base text-color surface-overlay p-2 border-1 border-solid surface-border border-round appearance-none outline-none focus:border-primary w-full" />
                </div>
                <div class="field col-12 md:col-12">
                    <label><span>Peralatan</span></label>
                    <InputText id="name" v-model="formCopy.attributes.peralatan"
                        class="text-base text-color surface-overlay p-2 border-1 border-solid surface-border border-round appearance-none outline-none focus:border-primary w-full" />
                </div>
                <div class="field col-12 md:col-12">
                    <label><span>Rincian Pekerjaan</span></label>
                    <InputText id="name" v-model="formCopy.attributes.rincian"
                        class="text-base text-color surface-overlay p-2 border-1 border-solid surface-border border-round appearance-none outline-none focus:border-primary w-full" />
                </div>
            </div>
        </form>
        ...
        async handleSubmit() {
            ...
            try {
                ...
                let data = {
                    ...
                    izinMarchandID: {
                    id: selectedID // Include the selected Internet Rate ID
                }
                };
                ...
                if (this.form.id == null) {
                    res = (await backend.kerjaTable.newEntry(requestData));
                } else {
                    res = (await backend.kerjaTable.updateByID(idForm, requestData));
                }
