<template>
  <div class="col-12 offset-10">
    ...
    <div class="flex justify-content-start mb-4">
      ...
      <span>
        <Dropdown v-model="selectedStatus" :options="statusfilter" placeholder="Select a City"
    ...
    <DataTable ref="dt" :filters="filters" :value="formData" selectionMode="single"
      @row-select="showModal('edit', $event.data)" responsiveLayout="scroll" sortField="id" :sortOrder="1"
...
import formModal from '@/components/modal/CreateCCForm.vue';
...
export default {
  ...
  methods: {
    ...
    async getAllForms() {
      try {
        let response = await backend.ccForm.findAll();
