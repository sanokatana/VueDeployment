<template>
  <div class="col-12 offset-10">
    <div class="flex justify-content-between align-items-center">
      <h1 class="text-ggi-red">Panel Form - Pemesanan Kartu Akses Masuk Pemilik / Penghuni / Warga</h1>
    </div>
    ...
    <DataTable :value="formData" v-model:selection="selectedForm" selectionMode="single" @row-select="showModal('edit')"
      ...
      <Column field="attributes.blok" header="Blok">
      </Column>
      <Column field="attributes.nounit" header="No. Unit">
      </Column>
...
import formModal from '@/components/modal/CreateKartuBaruForm.vue';
...
export default {
  ...
  methods: {
    ...
    async getAllForms() {
      try {
        let response = await backend.pemesananKartu.findAll();
