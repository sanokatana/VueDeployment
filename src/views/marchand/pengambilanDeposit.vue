<template>
    <div class="col-12 offset-10">
      <div class="flex justify-content-between align-items-center">
        <h1 class="text-ggi-red">Panel Form - Pengambilan Refund Deposit Fitout</h1>
      </div>
      ...
      <DataTable ref="formModal" :value="formData" v-model:selection="selectedForm" selectionMode="single" @row-select="handleRowSelect"
        ...
        <Column field="attributes.lantai" header="Lantai-Unit" :showFilterMenu="false">
        </Column>
        <Column field="attributes.namapenyewa" header="Nama Penyewa" :showFilterMenu="false">
        </Column>
        <Column field="attributes.statustenant" header="Status Toko" :showFilterMenu="false">
        </Column>
      ...
      <formModal ref="formModalRef" v-if="displayModal" v-model:displayModal="displayModal" v-model:form="selectedForm" :refreshList="getAllForms" :action="action" />
    </div>
  </template>
  ...
  import formModal from '@/components/modal/Marchand/CreatePengambilanForm.vue';
  ...
  export default {
    ...
    methods: {
      async getAllForms() {
        try {
          let response = await backend.pengambilanRefund.findAll();
      ...
      handleRowSelect() {
        ...
        if (this.$refs.formModalRef) {
          this.$refs.formModalRef.callGetAllFormulirs();
        }
      },
      showModal(action) {
        this.action = action;
